...
- **Literal and class scanning are dramatically faster on ARM64 (NEON).** Locating candidate
  positions inside a SIMD register is a single instruction on x86-64 but has no direct equivalent on
  ARM64, where it was previously emulated on *every* 16-byte block of input — even the vast majority
...
unbounded gap between two interior literals, case-insensitive unbounded alternations, …) and the
non-ASCII Unicode character-class decode path.
