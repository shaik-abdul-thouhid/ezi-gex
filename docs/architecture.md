...
> needles ≤ 4 bytes (`std.mem.findPos`), the common "the"/"http" sizes. **Since 0.6.2** the scan
> processes four chunks per iteration (after a short single-chunk warm-up, so a densely matching
> needle still returns at once) and adds a **third** probe byte for short all-common needles like
> `the` — together bringing single-literal scans to `rust/regex` parity on ARM64 (NEON).
...
| ✅ *(0.6.0)* | **Case-insensitive alternation → ASCII-folding Teddy** (`teddy.compileFoldAlloc`, `auto.caseiAlternationSet`, `Filter.prefix_set_fold_ascii`) — a top-level casei alternation (`(?i:Sherlock\|Holmes\|Watson)`, `(?i:Sher[a-z]+\|Hol[a-z]+)`) used to get **no** prefilter (folding turns each branch's leading letters into classes, so the fixed-leading-literal `prefix_set` declined). The Teddy nibble masks now carry **both ASCII cases** of each letter and verify folds case, so `auto` builds **one needle per branch** (not the case-variant set's `2^k` cartesian product) and a casei alternation stays inside `MAX_PREFIX_BRANCHES`. Non-ASCII fold members (`s`→`ſ`, `k`→KELVIN) fan out so the set is a sound necessary prefix; the automaton confirm enforces full matching | done | `(?i:Sherlock\|Holmes\|Watson)` ~14× → **faster than Rust**, `(?i:Sher[a-z]+\|Hol[a-z]+)` ~15× → ~6.4× (Sherlock geomean 1.64 → 1.50) |
...
| ✅ *(0.6.2)* | **NEON movemask gate** (`simd.cheap_movemask`) — `memmem`/`teddy`/`classscan` locate candidate lanes by `@bitCast`ing a bool vector to an integer; one instruction on x86-64, but emulated on aarch64 NEON and previously paid on every block. The scanners now gate it behind a cheap `@reduce(.Or)` any-hit test, paying the emulation only on a block that actually hit | done | restores literal/class throughput on ARM64; Sherlock geomean ~4.3 → ~1.7 |
| ✅ *(0.6.2)* | **`memmem` 4× unroll + third probe** (`engine/memmem.zig`) — the substring scan issues four 16-byte chunks per iteration (with a short single-chunk warm-up so dense matches return at once), and adds a third probe byte for short all-common needles (`the`) where the two-byte filter leaves too many candidates (the probes then cover the whole needle, so the verify is elided) | done | single-literal scans reach `rust/regex` parity on ARM64 (`Sherlock` 1.6× → 1.1×, `no-match-*` faster than Rust, `the` 2.5× → 1.7×); Sherlock geomean ~1.56 → ~1.45 |
| ✅ *(0.6.2)* | **Leading multi-prefix alternation jump-and-confirm** (`auto.runByteDfa`) — a top-level alternation whose branches each begin with a literal but are separated by an unbounded gap (`Holmes…Watson\|Watson…Holmes`) jumps prefix-to-prefix and confirms locally with a reach budget, instead of one native pass over the whole input | done | `holmes-coword-watson` ~21× → ~1.7× vs Rust |
